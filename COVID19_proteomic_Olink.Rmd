...
set.seed(100221)
rf_data_full <- cbind(full_model, training_indicies)
...
```{r}
ntree_values <- c(seq(from = 50, to = 1000, by = 50))
nodesize_values <- c(seq(from = 3, to = 15, by = 1))
maxnode_values <- c(seq(from = 5, to = 20, by = 5))
...
accuracy_train <- sum(predictions_train == rf_data_full_train$Acuity_max) / nrow(rf_data_full_train)
accuracy_test <- sum(predictions_test == rf_data_full_test$Acuity_max) / nrow(rf_data_full_test)
results <- rbind(results, data.frame(ntree = ntree, nodesize = nodesize, maxnodes = maxnodes, accuracy_train = accuracy_train, accuracy_test = accuracy_test))
