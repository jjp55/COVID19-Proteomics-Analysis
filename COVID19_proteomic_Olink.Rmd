...
```{r}
ntree_values <- c(seq(from = 100, to = 800, by = 100))
...
#Graph accuracy based on random forest parameters
```{r}
ggplot(results, aes(x = ntree, y = as.numeric(accuracy_test), color = as.factor(nodesize))) +
  geom_point() +
  xlab(expression("Number of Trees")) +
  ylab(expression("Test Accuracy")) +
  theme(axis.text = element_text(size = 14), axis.title = element_text(size = 20), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))
```
...
for (i in 1:100) {
rf_model_full <- randomForest(as.factor(Acuity_max) ~ ., data = rf_data_full_train, importance = TRUE, ntree = 375, nodesize = 9, maxnodes = 10)
